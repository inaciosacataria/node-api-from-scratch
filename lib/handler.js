...
//Tokens put
//require data id and extend
handlers._tokens.put = function (data, callback) {
    //get query string tokenObject
    var id = typeof (data.payload.id) == 'string'
        && data.payload.id.trim().length >= 10 ?
    ...
    if (id && extend) {
        ...
        //loookup the token object
        _data.read('tokens', id, function (err, tokenData) {
            if (!err && tokenData) {
               //make sure the token isnt already expired
                if (tokenData.expires > Date.now()) {
                    // set de experation an hour from now
                    tokenData.expires = Date.now() * 1000 * 60 * 60;
                    _data.update('tokens', id, tokenData, function (err) {
                        if (!err) {
                                callback(200)
                        } else {
                            callback(500, { 'Error': 'Could not update th tokens experation' }); 
                        }
                    });
                } else {
                    callback(400, { 'error': 'Token expired and cannote be extended' });
    ...
    if (id) {
        _data.read('tokens', id, function (err, data) {
            if (!err && data) {
                ...
                callback(400, { 'error': ' err Could not find specified token' });
            }
        });
